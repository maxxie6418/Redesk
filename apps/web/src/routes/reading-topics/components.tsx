...
export function CompactSelect({ options }: { options: string[] }) {
  return <FilterSelect value={options[0] ?? ''} onChange={() => {}} options={options.map((option) => ({ value: option, label: option }))} />;
}
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button variant="outline" size="sm">编辑话题</Button>
          <Button size="sm">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
        <aside className="space-y-4 lg:col-span-3">
          <SectionPanel title="关联书籍" className="rounded-lg p-4">
            ...
            <button type="button" className="mt-3 h-8 w-full rounded-md border border-dashed border-input text-xs text-muted-foreground transition-colors hover:border-foreground hover:text-foreground">
              + 添加书籍
            </button>
          </SectionPanel>
