...
- [ ] Task 9: 用户管理界面
  - [ ] SubTask 9.1: 用户列表显示权限级别
  - [ ] SubTask 9.2: 创建用户表单添加权限级别选择（默认 `use`）
  - [ ] SubTask 9.3: 更新用户支持修改权限级别

- [ ] Task 10: ProtectedShell 改造
  - [ ] SubTask 10.1: 支持匿名访问公开书架
  - [ ] SubTask 10.2: 根据权限级别控制页面访问

- [ ] Task 11: 书籍详情页 UI 改造
  - [ ] SubTask 11.1: 根据权限级别隐藏/显示文件相关元素
  - [ ] SubTask 11.2: 浏览用户不显示「打开阅读器」「下载」按钮
  - [ ] SubTask 11.3: 高亮列表区分「我的」和「其他人」

- [ ] Task 12: 书架页面改造
  - [ ] SubTask 12.1: 未登录用户可看到公开书籍列表
  - [ ] SubTask 12.2: 未登录用户点击书籍跳转登录页（而非详情页）
...
- [ ] Task 13: 权限校验验证
  - [ ] SubTask 13.1: 验证匿名用户只能看公开书架列表
  - [ ] SubTask 13.2: 验证浏览用户不能打开阅读器
  - [ ] SubTask 13.3: 验证阅读用户可写高亮但不能上传书籍
  - [ ] SubTask 13.4: 验证使用用户完整功能
  - [ ] SubTask 13.5: 验证管理员不受权限限制
